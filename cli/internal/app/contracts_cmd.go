...
const contractsUsage = `usage:
  nomos contracts status --repo-root <dir> [--out <report.json>] [--accept <id> --new-version <v>]
...
func runContractsStatus(args []string, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "contracts status: OK — %d contract(s): %v; %d compatibility read(s) → %s\n", rep.Total, rep.ByStability, rep.CompatReads, *out)
	return 0
}
