...
| `v1.0` | Stable product release candidate (planned as NRT-023..028 in [29](29-post-alpha-release-issue-list.md#v100---stable-product-release-candidate)). | Support model, compatibility policy, security process, stable contracts and customer integration guide are complete. A `v1.0` product version does not by itself establish validation for a regulated intended use; that status comes from roadmap 28. |
