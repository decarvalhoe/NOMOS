...
Facet vocabularies are validated as SKOS with standard SHACL (#580, VRC-44): `scripts/facet_shacl_gate.py` emits each pack's vocabulary as a real SKOS/OWL graph and validates it against `specs/shacl/facet-ontology.shapes.ttl` in CI. What this adds to the Go pack gate is not a second opinion on the same rules but a **portable** one: the shapes are standard SHACL over a standard graph, so pySHACL, Apache Jena or TopBraid render the same verdict on the same bytes, and the check is reproducible outside NOMOS. The shapes require a scheme that declares which axes are disjoint (declaring none would make orthogonality unfalsifiable, and that case is itself refused), an axis anchored to exactly one BFO root and one IOF class as real IRIs and carrying at least one term, and a concept with exactly one preferred label, scheme, axis, BFO mapping, IOF mapping and a pack-namespaced id. Orthogonality is a SPARQL constraint because it relates three nodes: a term sitting on two axes declared disjoint fails, naming the term and both axes. Both shipped verticals conform (built-environment 2 axes/10 concepts, eu-ai-act 3 axes/14 concepts). This claims conformance of the emitted graph to the shapes; it claims nothing about whether the BFO and IOF anchors chosen for a term are ontologically apt, which is a modelling judgement no validator makes. SKOS authoring and serving tooling (VocBench, Skosmos) is not implemented, so the capability stays declared `sidecar` rather than `real`.

Computable atoms run on a borrowed substrate, and NOMOS builds no rule engine (#578, VRC-42): `nomos rule exec` hands each ```formula block, verbatim and with its source trace, to an external process over a versioned JSON protocol, and records what comes back. `cli/internal/ruleexec` contains no evaluator, no arithmetic and no expression parser; with no substrate configured nothing is produced at all — never a default, never a zero — and a substrate that is absent, crashed, timed out or off-contract fails closed with no partial record. A result for an atom nobody asked about, a duplicate answer, a missing answer, a `computed` with no value, an `unsupported` with no reason or carrying a value: each is refused. Every value travels with the expression it came from and its canonical ref, file and line range, so a computed number is traceable to the text. The process boundary is what the licence register's `process_api_boundary` policy now means in practice: NOMOS spawns a process and imports, links and vendors nothing, no engine is named anywhere in the Go code (a wiring-matrix probe keeps it so), and the reference adapter imports no third-party module either. What is shipped is the boundary and a protocol-conformance fixture that evaluates one deliberately trivial arithmetic shape and answers `unsupported` for everything else — **no real rule engine is integrated**, and integrating one still requires the review the licence register demands. Nothing here is an opinion on whether this boundary satisfies the AGPL for any given engine.
